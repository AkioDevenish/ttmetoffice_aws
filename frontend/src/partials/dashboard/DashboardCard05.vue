<template>
  <div class="flex flex-col col-span-full sm:col-span-6 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
    ...
    <div v-else-if="!processedData.datasets[0].data.length" class="text-center text-gray-500 p-4">No data available for Port of Spain station</div>
    <div v-else>
      <RealtimeChart 
        :data="processedData" 
        :width="595" 
    ...
    const metricOptions = [
      ...
      try {
        ...
        if (Array.isArray(response.data) && response.data.length > 0) {
          const Port_of_SpainDataPoints = response.data.filter(station => station.name.toLowerCase() === 'port of spain');
          if ( Port_of_SpainDataPoints.length > 0) {
            console.log(' Port of Spain data points found:',  Port_of_SpainDataPoints);
            rawData.value =  Port_of_SpainDataPoints;
            lastUpdated.value = new Date( Port_of_SpainDataPoints[ Port_of_SpainDataPoints.length - 1].recorded_at);
          } else {
            console.error('No Port of Spain  data points found in the data');
            error.value = 'No  Port of Spain data points found in the data';
          }
        } else {
