<template>
  <div class="flex flex-col col-span-full sm:col-span-6 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
    ...
    <div v-if="loading && piacroTimeline.length === 0" class="flex justify-center items-center h-64">Loading...</div>
    <div v-else-if="error && piacroTimeline.length === 0" class="text-center text-red-500">{{ error }}</div>
    <div v-else>
      <RealtimeChart v-if="chartData" :data="chartData" :width="595" :height="248" :selectedMetric="selectedMetric" stationName="Piacro" />
...
export default {
  ...
  setup() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(response.data) && response.data.length > 0) {
          const piacroDataPoints = response.data.filter(station => station.name.toLowerCase() === 'piacro');
          if (piacroDataPoints.length > 0) {
            console.log('Piacro data points found:', piacroDataPoints);
            piacroTimeline.value = piacroDataPoints.sort((a, b) => new Date(a.recorded_at) - new Date(b.recorded_at)).slice(-24);
            lastUpdated.value = new Date(piacroDataPoints[piacroDataPoints.length - 1].recorded_at);
          } else {
            console.error('No Piacro data points found in the data');
            error.value = 'No Piacro data points found in the data';
          }
        } else {
    ...
    watch([piacroTimeline, selectedMetric], () => {
      if (piacroTimeline.value.length > 0 && selectedMetric.value) {
        const labels = piacroTimeline.value.map(entry => new Date(entry.recorded_at).toLocaleTimeString());
        const data = piacroTimeline.value.map(entry => parseFloat(entry[selectedMetric.value]));
