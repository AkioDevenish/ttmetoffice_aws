<template>
  <div class="flex flex-col col-span-full sm:col-span-6 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
    ...
    <div v-else-if="!processedData.datasets[0].data.length" class="text-center text-gray-500 p-4">No data available for Piarco station</div>
    <div v-else>
      <RealtimeChart 
        :data="processedData" 
        :width="595" 
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(response.data) && response.data.length > 0) {
          const piarcoDataPoints = response.data.filter(station => station.name.toLowerCase() === 'piarco');
          if (piarcoDataPoints.length > 0) {
            console.log('Piarco data points found:', piarcoDataPoints);
            rawData.value = piarcoDataPoints;
            lastUpdated.value = new Date(piarcoDataPoints[piarcoDataPoints.length - 1].recorded_at);
          } else {
            console.error('No Piarco data points found in the data');
            error.value = 'No Piarco data points found in the data';
          }
        } else {
