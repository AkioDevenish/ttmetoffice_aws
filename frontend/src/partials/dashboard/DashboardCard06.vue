<template>
  <div class="flex flex-col col-span-full sm:col-span-6 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
    ...
    <div v-if="loading && guayaguayareTimeline.length === 0" class="flex justify-center items-center h-64">Loading...</div>
    <div v-else-if="error && guayaguayareTimeline.length === 0" class="text-center text-red-500">{{ error }}</div>
    <div v-else>
      <RealtimeChart v-if="chartData" :data="chartData" :width="595" :height="248" :selectedMetric="selectedMetric" stationName="Guayaguayare" />
...
export default {
  ...
  setup() {
    const guayaguayareTimeline = ref([]);
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(response.data) && response.data.length > 0) {
          const guayaguayareDataPoints = response.data.filter(station => station.name.toLowerCase() === 'guayaguayare');
          if (guayaguayareDataPoints.length > 0) {
            console.log('Guayaguayare data points found:', guayaguayareDataPoints);
            guayaguayareTimeline.value = guayaguayareDataPoints.sort((a, b) => new Date(a.recorded_at) - new Date(b.recorded_at)).slice(-24);
            lastUpdated.value = new Date(guayaguayareDataPoints[guayaguayareDataPoints.length - 1].recorded_at);
          } else {
            console.error('No Guayaguayare data points found in the data');
            error.value = 'No Guayaguayare data points found in the data';
          }
        } else {
    ...
    watch([guayaguayareTimeline, selectedMetric], () => {
      if (guayaguayareTimeline.value.length > 0 && selectedMetric.value) {
        const labels = guayaguayareTimeline.value.map(entry => new Date(entry.recorded_at).toLocaleTimeString());
        const data = guayaguayareTimeline.value.map(entry => parseFloat(entry[selectedMetric.value]));
